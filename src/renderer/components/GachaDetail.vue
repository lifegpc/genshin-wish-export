<template>
  ...
  <p class="text-gray-600 text-xs mb-1" v-if="detail.ssrPos.length">
    ...
    >
      ...
      <span v-if="type == '11' && item[4] === null" class="text-red-600">[未知]</span>
    </span>
  </p>
  ...
  <p v-if="type == '11' && detail.ssrPos.length" class="text-gray-600 text-xs">5星统计{{colon}}
  ...
  <p v-if="type == '11' && detail.ssrPos.length && detail.upc" class="text-gray-600 text-xs">5星UP平均出货次数{{colon}}<span class="text-green-600">{{ avgUp5(detail.ssrPos) }}</span></p>
</template>
